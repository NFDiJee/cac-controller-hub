...
function initSubTabs() {
  document.querySelectorAll('.sub-nav-btn[data-sub]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.sub === 'stats' && selectedNodeId) loadNodeStats();
    });
  });
}
...
async function loadNodeStats() {
  if (!selectedNodeId) return;
  try {
    nodeStatsData = await proxyGet('stats?top=' + (topListLimit === 9999 ? 99999 : topListLimit));
    renderStatsTab();
  } catch (err) { console.error(err); }
}

function renderStatsTab() {
  if (!nodeStatsData) return;
...
function renderStatsOverview() {
  const inv = nodeStatsData.inventory;
  const totalPlayTime = statsDuration(nodeStatsData.totalPlayTimeSec);
  const totalLibTime = statsDuration(inv.totalDurationSeconds);
  ...
  if (nodeStatsData.topCDs.length) {
    html += `<div class="card" style="margin-top:16px"><div class="card-header"><span class="card-title">${t('stats.topCDsQuick')}</span></div>`;
    html += nodeStatsData.topCDs.slice(0, 5).map((c, i) => {
      const coverSrc = c.cover_url ? `/api/nodes/${selectedNodeId}/cover/${c.cover_url.replace(/^\/covers\//, '')}` : '';
      return `<div class="list-item" onclick="openCdModal(${c.slot})" style="cursor:pointer">
        <div class="top-rank">${i + 1}</div>
        ${coverSrc ? `<img class="top-cover" src="${coverSrc}" alt="">` : `<div class="top-cover-ph">${c.slot}</div>`}
        <div class="list-meta">
          <div class="list-primary">${esc(c.cd_title || 'CD ' + c.slot)}</div>
          <div class="list-secondary">${esc(c.cd_artist || '')} · ${c.play_count}x</div>
        </div>
      </div>`;
    }).join('');
    html += '</div>';
...
function renderStatsToplist() {
  let html = `<div class="stats-controls">
    <div class="btn-group">
      <button class="btn btn-sm ${topListView === 'cds' ? 'btn-accent' : 'btn-dim'}" onclick="topListView='cds';renderStatsTab()">${t('stats.topCDs')}</button>
      <button class="btn btn-sm ${topListView === 'tracks' ? 'btn-accent' : 'btn-dim'}" onclick="topListView='tracks';renderStatsTab()">${t('stats.topTracks')}</button>
    </div>
    <div class="btn-group">
      ${[10, 25, 50, 100].map(n => `<button class="btn btn-sm ${topListLimit === n ? 'btn-accent' : 'btn-dim'}" onclick="changeTopLimit(${n})">Top ${n}</button>`).join('')}
      <button class="btn btn-sm ${topListLimit === 9999 ? 'btn-accent' : 'btn-dim'}" onclick="changeTopLimit(9999)">${t('stats.all')}</button>
    </div>
  </div>`;

  if (topListView === 'cds') {
    const items = nodeStatsData.topCDs.slice(0, topListLimit);
...
async function changeTopLimit(n) {
  topListLimit = n;
  if (n > nodeStatsData.topCDs.length || n > nodeStatsData.topTracks.length) {
    nodeStatsData = await proxyGet('stats?top=' + (n === 9999 ? 99999 : n));
  }
  renderStatsTab();
}
  ...
  let html = `<div class="stats-controls">
    <div class="btn-group">
      ${['daily', 'weekly', 'monthly', 'yearly'].map(p =>
        `<button class="btn btn-sm ${activityPeriod === p ? 'btn-accent' : 'btn-dim'}" onclick="activityPeriod='${p}';renderStatsTab()">${t('stats.' + p)}</button>`
      ).join('')}
    </div>
  </div>`;

  const data = nodeStatsData[activityPeriod] || [];
  if (!data.length) return html + `<div class="empty-state">${t('stats.noData')}</div>`;
...
function renderStatsInventory() {
  ...
  let html = `<div class="stats-grid">
    <div class="stat-card"><div class="stat-value">${inv.totalCDs}</div><div class="stat-label">${t('stats.cds')}</div></div>
    <div class="stat-card"><div class="stat-value">${inv.totalTracks}</div><div class="stat-label">${t('stats.tracks')}</div></div>
    <div class="stat-card"><div class="stat-value">${statsDuration(inv.totalDurationSeconds)}</div><div class="stat-label">${t('stats.totalDuration')}</div></div>
    <div class="stat-card"><div class="stat-value">${inv.totalPlaylists}</div><div class="stat-label">${t('stats.playlists')}</div></div>
  ...
  if (inv.genreDistribution && inv.genreDistribution.length) {
    ...
    html += inv.genreDistribution.map(g => `
      <div class="genre-bar-row">
        <span class="genre-bar-label">${esc(g.genre || t('stats.unknown'))}</span>
